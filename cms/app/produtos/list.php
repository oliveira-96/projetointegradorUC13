<?php
    ...
    if (!empty($busca)) {
        ...
        $campos[] = "referencia LIKE '%{$search}%'";
        $campos[] = "codigo_barras LIKE '%{$search}%'";
    ...
    $query = mysqli_query($db, "SELECT * FROM produtos" . (!empty($busca) ? " WHERE ({$search})" : '') . " ORDER BY nome ASC LIMIT {$inicio}, {$limite}");
?>
...
<div class="row">
	<div class="col-lg-12">
                    ...
                    if (mysqli_num_rows($query)) {
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    <?= (!empty($item['nome']) ? $item['nome'] : '') . (!empty($item['referencia']) ? " <small>| Ref.: {$item['referencia']}" : ''); ?>
                                </td>
                                <td style="width: 15%;">
                                    R$ <?= $util->formatDecimalMoeda((!empty($item['preco_venda']) ? $item['preco_venda'] : 0)); ?>
                                </td>
                                <td style="width: 10%;">
                                    <?= (!empty($item['status']) ? 'Disponível' : 'Indisponível'); ?>
                                </td>
        ...
        <?php
            ...
            $query = mysqli_query($db, "SELECT count(*) AS cont FROM produtos" . (!empty($busca) ? " WHERE ({$search})" : ''));
            if ($query) {
