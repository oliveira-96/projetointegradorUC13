<?php
    ...
    if (!empty($busca)) {
        ...
        $campos = array();
        $campos[] = "departamento LIKE '%{$search}%'";
    ...
    $query = mysqli_query($db, "SELECT * FROM departamentos" . (!empty($busca) ? " WHERE ({$search})" : '') . " ORDER BY departamento ASC LIMIT {$inicio}, {$limite}");
?>
...
<div class="row">
	<div class="col-lg-12">
                    ...
                    if (mysqli_num_rows($query)) {
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    <?= (!empty($item['departamento']) ? $item['departamento'] : ''); ?>
                                </td>
                                <td style="width: 10%;">
                                    <?= (!empty($item['status']) ? 'Habilitado' : 'Desabilitado'); ?>
                                </td>
        ...
        <?php
            ...
            $query = mysqli_query($db, "SELECT count(*) AS cont FROM departamentos" . (!empty($busca) ? " WHERE ({$search})" : ''));
            if ($query) {
