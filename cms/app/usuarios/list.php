<?php
    ...
    $query = mysqli_query($db, "SELECT * FROM usuarios" . (!empty($busca) ? " WHERE ({$search})" : '') . " ORDER BY id ASC LIMIT {$inicio}, {$limite}");
?>
...
<div class="row">
	<div class="col-lg-12">
        ...
        <?php
            ...
            $query = mysqli_query($db, "SELECT count(*) AS cont FROM usuarios" . (!empty($busca) ? " WHERE ({$search})" : ''));
            if ($query) {
